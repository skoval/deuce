fetch_draw <- function(tournament, year){
...
-63L))
    ...
    if (length(lines) == 0) 
        ...
        if (type == "Grand Slam") 
            ...
            Points <- c(500, 300, 180, 90, 45, 0) else if (type == "250" & maxround == 6)
            Points <- c(250, 150, 90, 45, 20, 10, 0) else Points <- c(250, 150, 90, 45, 20, 0)
        ...
        Result$year <- year
        Result$Seed <- Ranks[match(Result$Player, names(Ranks))]
